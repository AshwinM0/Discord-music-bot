...
class MusicControlView(discord.ui.View):
    ...
    async def toggle_play(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if not voice:
            embed = discord.Embed(description="Nothing is playing right now.", color=discord.Color.orange())
        ...
        if voice.is_playing():
             ...
             embed = discord.Embed(description=f"⏸ Paused{title}.", color=discord.Color.orange())
             ...
             if current and current.get("paused_at"):
                 ...
                 current["paused_at"] = 0
             embed = discord.Embed(description=f"▶ Resumed{title}.", color=discord.Color.green())
             await interaction.response.send_message(embed=embed)
        else:
             embed = discord.Embed(description="Nothing is playing right now.", color=discord.Color.orange())
    ...
    async def skip(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if voice and (voice.is_playing() or voice.is_paused()):
            ...
            voice.stop()
            embed = discord.Embed(description=f"⏭ Skipped{skipped_text}.", color=discord.Color.green())
            await interaction.response.send_message(embed=embed)
        else:
             embed = discord.Embed(description="Nothing is playing right now.", color=discord.Color.orange())
             await interaction.response.send_message(embed=embed, ephemeral=True)
...
class DuplicateConfirmView(discord.ui.View):
    ...
    async def on_timeout(self) -> None:
        if not self.responded:
            embed = discord.Embed(description="⏰ Duplicate confirmation timed out.", color=discord.Color.greyple())
            try:
    ...
    async def confirm(self, interaction: discord.Interaction, button: discord.ui.Button):
        if interaction.user != self.ctx.author:
            return await interaction.response.send_message(
                embed=discord.Embed(description="Only the person who queued this can confirm.", color=discord.Color.red()),
                ephemeral=True,
            )
        self.responded = True
    ...
    async def cancel(self, interaction: discord.Interaction, button: discord.ui.Button):
        if interaction.user != self.ctx.author:
            return await interaction.response.send_message(
                embed=discord.Embed(description="Only the person who queued this can cancel.", color=discord.Color.red()),
                ephemeral=True,
            )
        self.responded = True
        embed = discord.Embed(description="❌ Cancelled — duplicate not added.", color=discord.Color.greyple())
...
class Music(commands.Cog):
    ...
    async def _ensure_voice(self, ctx: commands.Context) -> discord.VoiceClient | None:
        ...
        if not ctx.author.voice:
            if voice:
                await self._send_embed(ctx, "Join the VC first.", color=discord.Color.red())
            else:
                await self._send_embed(ctx, "Join a voice channel first.", color=discord.Color.red())
            return None
    ...
    async def _play_next(self, ctx: commands.Context) -> None:
        ...
        if queue:
            ...
            embed = discord.Embed(
                description=f"🎶 Now playing: **{track_text}**",
                color=discord.Color.green(),
            )
            if queue:
                embed.set_footer(text=f"{len(queue)} track{'s' if len(queue) != 1 else ''} remaining in queue")
            ...
            if voice and not voice.is_playing() and not voice.is_paused() and not queue:
                await self._send_embed(ctx, "Nothing was played in the last 2 minutes. Leaving.")
                await voice.disconnect()
    ...
    async def join(self, ctx: commands.Context) -> None:
        ...
        if not ctx.author.voice:
            if voice:
                await self._send_embed(ctx, "I'm already in a VC and you're not in one.", color=discord.Color.red())
            else:
                await self._send_embed(ctx, "Join a voice channel first.", color=discord.Color.red())
            return
        ...
        if voice is None:
            ...
            await self._send_embed(ctx, f"Joined **{author_channel.name}**")
        elif voice.channel == author_channel:
            await self._send_embed(ctx, "I'm already in your VC.", color=discord.Color.orange())
        else:
            await voice.disconnect()
            await author_channel.connect()
            logger.info("Moved to %s in guild %s", author_channel, ctx.guild.id)
            await self._send_embed(ctx, f"Moved to **{author_channel.name}**")
    ...
    async def leave(self, ctx: commands.Context) -> None:
        ...
        if voice:
            ...
            await self._send_embed(ctx, "👋 Left the voice channel.")
        else:
            await self._send_embed(ctx, "I'm not in a VC.", color=discord.Color.red())
    ...
    async def _enqueue(self, ctx: commands.Context, query: str) -> None:
        ...
        if voice.is_playing() or voice.is_paused():
            ...
            await self._send_embed(
                ctx,
                f"📥 **{query}** added to queue at position **#{position}**.",
                color=discord.Color.green(),
    ...
    async def play(self, ctx: commands.Context, *, query: str) -> None:
        ...
        if len(queue) >= MAX_QUEUE_SIZE:
            await self._send_embed(
                ctx,
                f"Queue is full (**{MAX_QUEUE_SIZE}** tracks max). Remove or clear some tracks first.",
        ...
        if in_queue or is_playing:
            ...
            embed = discord.Embed(
                description=f"⚠️ **{query}** is already {'playing' if is_playing else 'in the queue'}.\nDo you want to add it again?",
    ...
    async def skip(self, ctx: commands.Context) -> None:
        ...
        if voice and (voice.is_playing() or voice.is_paused()):
            ...
            await self._send_embed(ctx, f"⏭ Skipped{skipped_text}.", color=discord.Color.green())
        else:
            await self._send_embed(ctx, "Nothing is playing right now.", color=discord.Color.orange())
    ...
    async def pause(self, ctx: commands.Context) -> None:
        ...
        if voice and voice.is_playing():
            ...
            if current:
                ...
                title = ""
            await self._send_embed(ctx, f"⏸ Paused{title}.", color=discord.Color.orange())
        elif voice and voice.is_paused():
            await self._send_embed(ctx, "Already paused. Use `resume` to continue.", color=discord.Color.orange())
        else:
            await self._send_embed(ctx, "Nothing is playing right now.", color=discord.Color.orange())
    ...
    async def resume(self, ctx: commands.Context) -> None:
        ...
        if voice and voice.is_paused():
            ...
            if current:
                ...
                title = ""
            await self._send_embed(ctx, f"▶ Resumed{title}.", color=discord.Color.green())
        elif voice and voice.is_playing():
            await self._send_embed(ctx, "Already playing!", color=discord.Color.orange())
        else:
            await self._send_embed(ctx, "Nothing to resume.", color=discord.Color.orange())
    ...
    async def stop(self, ctx: commands.Context) -> None:
        ...
        if voice and (voice.is_playing() or voice.is_paused()):
            setattr(self, f'_skip_req_{ctx.guild.id}', True)
            voice.stop()
            await self._send_embed(ctx, "🟥 Stopped and cleared the queue.", color=discord.Color.red())
        else:
            await self._send_embed(ctx, "Not playing anything right now.", color=discord.Color.orange())
    ...
    async def now_playing_cmd(self, ctx: commands.Context) -> None:
        ...
        status = "⏸ Paused" if voice.is_paused() else "🎶 Playing"
        embed = discord.Embed(
    ...
    async def queue(self, ctx: commands.Context) -> None:
        ...
        if not q and not current:
            await self._send_embed(ctx, "The queue is empty and nothing is playing.", color=discord.Color.orange())
            return
        ...
        if current:
            ...
            parts.append(f"{icon} **Now:** {track_text}\n")

        if q:
            entries = [f"`{i+1}.` {track}" for i, track in enumerate(q)]
            parts.append("\n".join(entries))
        ...
        embed.set_footer(text=f"{len(q)} track{'s' if len(q) != 1 else ''} queued | Loop: {self.loop_modes.get(guild_id, 'off')}")
        await ctx.send(embed=embed)
    ...
    async def volume(self, ctx: commands.Context, vol: int) -> None:
        ...
        if vol < 0 or vol > 100:
            await self._send_embed(ctx, "Volume must be between 0 and 100.", color=discord.Color.red())
            return
        ...
        await self._send_embed(ctx, f"🔊 Volume set to **{vol}%**.", color=discord.Color.green())
    ...
    async def loop(self, ctx: commands.Context, mode: str = "") -> None:
        ...
        if mode not in ("off", "track", "queue"):
            await self._send_embed(ctx, "Usage: `<>loop [off | track | queue]`", color=discord.Color.orange())
            return
        
        self.loop_modes[ctx.guild.id] = mode
        icons = {"off": "➡", "track": "🔂", "queue": "🔁"}
        await self._send_embed(ctx, f"{icons[mode]} Loop mode set to **{mode}**.", color=discord.Color.green())
    ...
    async def shuffle(self, ctx: commands.Context) -> None:
        ...
        if len(q) < 2:
            await self._send_embed(ctx, "Not enough tracks in the queue to shuffle.", color=discord.Color.orange())
            return
            
        lst = list(q)
        random.shuffle(lst)
        self.queues[ctx.guild.id] = deque(lst)
        await self._send_embed(ctx, "🔀 Queue shuffled.", color=discord.Color.green())
    ...
    async def remove(self, ctx: commands.Context, position: int) -> None:
        ...
        if position < 1 or position > len(q):
            await self._send_embed(ctx, f"Invalid position. The queue has {len(q)} tracks.", color=discord.Color.red())
            return
            
        lst = list(q)
        removed = lst.pop(position - 1)
        self.queues[ctx.guild.id] = deque(lst)
        await self._send_embed(ctx, f"🗑️ Removed **{removed}** from the queue.", color=discord.Color.green())
    ...
    async def clear(self, ctx: commands.Context) -> None:
        ...
        q = self._get_queue(ctx.guild.id)
        if not q:
            await self._send_embed(ctx, "Queue is already empty.", color=discord.Color.orange())
            return
        q.clear()
        await self._send_embed(ctx, "🗑️ Queue cleared.", color=discord.Color.green())
